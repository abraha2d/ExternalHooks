...
def get_executables(dir):
    ...
    for fyle in os.listdir(dir):
        ...
        if os.stat(path)[stat.ST_MODE] & (stat.S_IXUSR|stat.S_IXGRP|stat.S_IXOTH):
...
if __name__ == '__main__':
    ...
    for exec_fyle in get_executables(sys.argv[1]):
        ret += subprocess.run([exec_fyle] + sys.argv[2:], input=stdin, universal_newlines=True).returncode
    sys.exit(ret)
